 ...
 * work correctly when this filter is used in the search-time analyzer.
 *
 * <p>
 ...
 * </ul>
 * One use for {@link MusicbrainzWordDelimiterGraphFilter} is to help match words with different
...
public final class MusicbrainzWordDelimiterGraphFilter extends TokenFilter {
    ...
    private final MusicbrainzWordDelimiterIterator iterator;
    ...
    /**
     * Creates a new MusicbrainzWordDelimiterGraphFilter
     *
     ...
     */
    public MusicbrainzWordDelimiterGraphFilter(TokenStream in, byte[] charTypeTable, int configurationFlags, CharArraySet protWords) {
        super(in);
        ...
        this.iterator = new MusicbrainzWordDelimiterIterator(
                charTypeTable, has(SPLIT_ON_CASE_CHANGE), has(SPLIT_ON_NUMERICS), has(STEM_ENGLISH_POSSESSIVE));
    }

    /**
     * Creates a new MusicbrainzWordDelimiterGraphFilter using {@link MusicbrainzWordDelimiterIterator#DEFAULT_WORD_DELIM_TABLE}
     ...
     * @param protWords If not null is the set of tokens to protect from being delimited
     */
    public MusicbrainzWordDelimiterGraphFilter(TokenStream in, int configurationFlags, CharArraySet protWords) {
        this(in, MusicbrainzWordDelimiterIterator.DEFAULT_WORD_DELIM_TABLE, configurationFlags, protWords);
    }
    ...
    private void bufferWordParts() throws IOException {
        ...
        if (iterator.isSingleWord()) {
            ...
            while (iterator.end != MusicbrainzWordDelimiterIterator.DONE) {
    ...
    public boolean incrementToken() throws IOException {
        while (true) {
            if (savedState == null) {
                ...
                if (iterator.end == MusicbrainzWordDelimiterIterator.DONE) {
    ...
    */
    ...
    private boolean shouldConcatenate(int wordType) {
        return (has(CATENATE_WORDS) && MusicbrainzWordDelimiterIterator.isAlpha(wordType)) || (has(CATENATE_NUMBERS) && MusicbrainzWordDelimiterIterator.isDigit(wordType));
    }
    ...
    private boolean shouldGenerateParts(int wordType) {
        return (has(GENERATE_WORD_PARTS) && MusicbrainzWordDelimiterIterator.isAlpha(wordType)) || (has(GENERATE_NUMBER_PARTS) && MusicbrainzWordDelimiterIterator.isDigit(wordType));
    }
    ...
    public String toString() {
        StringBuilder b = new StringBuilder();
        b.append("MusicbrainzWordDelimiterGraphFilter(flags=");
